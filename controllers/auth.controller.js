...
const { Parcel, DriverTrip, PickupOffer } = require('../models');
/**
...
exports.deleteAccount = async (req, res) => {
  try {
    ...
    // 1️⃣ ამ მომხმარებლის საკუთარი განცხადებები (როგორც sender)
    const ownParcels = await Parcel.find({ senderId: userId }).select('_id').lean();
    const ownParcelIds = ownParcels.map(p => p._id);

    // 2️⃣ სხვისი parcel-ები, რომლებიც ამ მომხმარებელს (როგორც driver-ს)
    //    აქვს მიღებული — არ იშლება, უბრუნდება pending-ს
    await Parcel.updateMany(
      { acceptedBy: userId, senderId: { $ne: userId } },
      { $set: { status: 'pending', acceptedBy: null, acceptedTrip: null, acceptedAt: null } }
    );

    // 3️⃣ სხვისი DriverTrip-ების acceptedShippings-იდან ამ მომხმარებლის
    //    parcel-ების refs-ის გასუფთავება (სანამ თვითონ parcel-ები წაიშლება)
    if (ownParcelIds.length > 0) {
      await DriverTrip.updateMany(
        { acceptedShippings: { $in: ownParcelIds } },
        { $pull: { acceptedShippings: { $in: ownParcelIds } } }
      );
    }

    // 4️⃣ მომხმარებლის გაგზავნილი განცხადებების წაშლა
    await Parcel.deleteMany({ senderId: userId });

    // 5️⃣ მომხმარებლის მგზავრობების წაშლა (driver)
    await DriverTrip.deleteMany({ driverId: userId });

    // 6️⃣ ყველა PickupOffer, სადაც ეს user sender ან driver როლშია
    await PickupOffer.deleteMany({
      $or: [{ senderId: userId }, { driverId: userId }]
    });

    // ⚠️ განზრახ არაფერს ვაკეთებთ Message/Conversation-ზე —
    // ჩათის ისტორია მომხმარებლის მოთხოვნით ხელუხლებელი რჩება.

    // 7️⃣ საბოლოოდ — User-ის ჩანაწერის წაშლა
    await User.findByIdAndDelete(userId);

    return res.status(200).json({
      success: true,
      message: 'ანგარიში და მასთან დაკავშირებული მონაცემები (გარდა ჩათისა) წარმატებით წაიშალა'
    });
  } catch (error) {
